...
export class ResultComponent implements OnInit {
  ...
  placeHolder =
    'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla ultricies nec nulla ut rutrum.';
  sourceHtml: string = `
    <!DOCTYPE html>
    <html lang="en">
    <head>
...
.navbar-5 {
  ...
  copy(type: number) {
    ...
    this.defaultCopyValues();
    if (type === CopyCategory.CSS) {
      this.cssCopied = true;
    } else if (type === CopyCategory.HTML) {
      this.htmlCopied = true;
    }
    this.copyTimeout = setTimeout(() => {
      this.defaultCopyValues();
    }, this.copyTimeoutValue);
  }
